...
/**
 * Richer parse for the bake pipeline: separates the trail polyline
 * (<trkpt>/<rtept>) from standalone named waypoints (<wpt>), and pulls the
 * trail's display name out of <trk><name> or <metadata><name>.
 *
 * @param {string} gpxText
 * @returns {{
 *   points: {lat:number, lon:number, ele:number|null}[],
 *   waypoints: {lat:number, lon:number, ele:number|null, name:string}[],
 *   name: string|null
 * }}
 */
export function parseGpx(gpxText) {
  ...
  while ((m = tagRe.exec(gpxText)) !== null) {
    ...
    if (tag === "wpt") {
      const nameMatch = body.match(/<name>\s*(?:<!\[CDATA\[)?([\s\S]*?)(?:\]\]>)?\s*<\/name>/i);
      waypoints.push({
        lat,
        lon,
        ele,
        name: nameMatch ? decodeEntities(nameMatch[1]) : "Waypoint",
      });
    } else {
      points.push({ lat, lon, ele });
    }
  }
  // A waypoints-only GPX (no track) still gets a polyline so planBake works.
  const trail = points.length >= 2 ? points : waypoints;
  return { points: trail, waypoints, name: extractGpxName(gpxText) };
}
...
function extractGpxName(gpxText) {
  for (const scope of [
    ...
    const block = gpxText.match(scope);
    if (!block) continue;
    const n = block[1].match(/<name>\s*(?:<!\[CDATA\[)?([\s\S]*?)(?:\]\]>)?\s*<\/name>/i);
    if (n && n[1].trim()) return decodeEntities(n[1].trim());
  }
  return null;
}
...
export function planBakeFromFile(path, opts) {
  ...
  const { points } = parseGpx(text); // track points only; wpt aren't the trail
